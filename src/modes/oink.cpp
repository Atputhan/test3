    ...
    while (pendingHsRead != pendingHsWrite) {
        ...
        if (idx >= 0) {
            ...
            if (pendingHandshakes[slot]->hasPMKID) {
                ...
                // CURSED: Jumpscare + marquee on PMKID capture
                memcpy(lastPMKID, pendingHandshakes[slot]->pmkid, 16);
                for (const auto& net : networks()) {
                    if (memcmp(net.bssid, pendingHandshakes[slot]->bssid, 6) == 0) {
                        strncpy(lastPMKIDSSID, net.ssid, 32);
                        lastPMKIDSSID[32] = 0;
                        cursed_pmkidMarqueeSet(lastPMKID, lastPMKIDSSID);
                        cursed_jumpscareTrigger();
                        hasLastPMKID = true;
                        break;
                    }
                }
